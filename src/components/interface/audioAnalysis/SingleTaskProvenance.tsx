  ...
  return (
    <g style={{ cursor: 'pointer' }}>
      {answer.provenanceGraph ? Object.entries(answer.provenanceGraph.nodes).map((entry) => {
        ...
        return <g key={nodeId} onClick={() => setCurrentNode(nodeId, node.createdOn, taskName)}><AnimatedCircle color={nodeId === currentNode ? 'cornflowerblue' : 'lightgray'} cx={xScale(node.createdOn)} cy={height / 2} r={5} /></g>;
      }) : null}
